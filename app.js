...
function pressOp(op){
  // If we just pressed '=', tokens is [result]. Always allow an op to be appended.
  if (afterEquals){ afterEquals=false; }
  // Case 1: replace existing trailing operator
  if (tokens.length>0 && typeof tokens[tokens.length-1]==="string"){
    tokens[tokens.length-1] = op; renderInput(); return;
  }
  // Case 2: if a complete current fraction is in the pads, commit it first
  const cur = currentFrac();
  if (cur){ tokens.push(cur); whole=num=den=""; }
  else {
    // If no current fraction, but we already have a left operand, allow pushing the operator
    if (tokens.length>0){
      tokens.push(op); renderInput(); return;
    } else {

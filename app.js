// app.js with denominator display fix and basic fraction ops
(function(){
  ...
  function toFraction(obj){
    ...
    const calcDen = d === 0 ? 1 : d; // keep UI as 0, safe math internally
    const sign = w < 0 ? -1 : 1;
    w = Math.abs(w);
  ...
  function renderResult(fr){
    ...
    const m = asMixed(fr);
    if (m.num === 0){ resultDisplay.textContent = `${m.whole}`; }
    else if (m.whole === 0){ resultDisplay.textContent = `${m.num}/${m.den}`; }
    else { resultDisplay.textContent = `${m.whole} ${m.num}/${m.den}`; }
  }
  ...
  document.querySelectorAll('[data-clear]').forEach(btn=>{
    btn.addEventListener('click', ()=>{
      ...
      if (t==='whole') left.whole = ""; if (t==='num') left.num = ""; if (t==='den') left.den = "";
      renderInput();
    });
  });

  document.getElementById('clearAll').addEventListener('click', ()=>{
    left.whole = left.num = left.den = ""; stored = null; pendingOp = null;
    renderInput(); renderResult(null);
  });

  function compute(a,b,op){ if(op==='+')return add(a,b); if(op==='-')return sub(a,b); if(op==='*')return mul(a,b); if(op==='/')return div(a,b); return b; }
